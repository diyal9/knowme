# workbench-flow-library Specification

## Purpose

让用户在同一工作台中发现、比较、复制、配置和启动专业固定管线与个人 Agent 工作流。
...
流程库 MUST 区分官方、团队、个人和派生流程，并单独显示执行后端；不得把执行后端当作流程类别。
...
流程库 MUST 以可筛选列表和详情面板呈现。列表 MUST 支持领域、来源、可用性和执行后端筛选；详情 MUST 显示输入、输出、Agent、步骤、质量门禁、readiness、最近运行和唯一主操作。
