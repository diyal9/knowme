...
工作台悬浮助理入口 MUST 默认以右下角的单一描边铃铛呈现，不得使用常驻的实心药丸底板、通用实心聊天气泡、品牌节点标记或厚重投影；入口 MUST 在浅色和深色主题中保持可识别，并保留原有快捷操作能力。状态提示 MUST 与铃铛共享视觉层级，不得同时出现两个相互竞争的强调红点。
...
- **AND** 状态提示 SHALL 贴近铃铛右上沿，不得游离在按钮左侧
