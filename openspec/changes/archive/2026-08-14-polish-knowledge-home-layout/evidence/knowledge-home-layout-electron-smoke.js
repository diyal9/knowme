...
const REPORT = path.join(__dirname, 'knowledge-home-layout-electron-smoke.json')
...
async function main() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'knowme-home-layout-'))
  ...
  try {
    ...
    const home = await window.evaluate(() => {
      ...
      const body = document.querySelector('.knowledge-desk-body')
      return {
        ...
        command: !!document.querySelector('.knowledge-desk-command'),
        bodyHeight: Math.round(body?.getBoundingClientRect().height || 0),
        deskHeight: Math.round(desk?.getBoundingClientRect().height || 0),
        text: desk?.textContent || '',
        hasHero: !!desk?.querySelector('h1'),
        graphCanvas: !!document.querySelector('canvas[id*="Graph"], .knowledge-graph-canvas'),
      }
    })
    checks.push({
      id: 'coherent-knowledge-workspace',
      pass: home.search && home.add && home.lint && home.browse && home.obsidian && home.tree && home.command && home.bodyHeight > 250 && !home.hasHero,
      detail: home,
    })
    checks.push({
      id: 'no-internal-graph-surface',
      pass: !home.graphCanvas && !/Fabric|织网|authority/.test(home.text),
    ...
    checks.push({
      id: 'search-remains-functional',
      pass: /智能检索|本地检索/.test(retrieval.engine) && retrieval.hitCount >= 1,
      detail: retrieval,
    })

    await window.screenshot({ path: path.join(SHOTS, 'knowledge-home-layout-desktop.png'), scale: 'css' })
    await window.setViewportSize({ width: 510, height: 820 })
    await window.waitForTimeout(300)
    const narrow = await window.evaluate(() => ({
      viewport: document.documentElement.clientWidth,
      bodyWidth: document.body.scrollWidth,
      toolbarVisible: !!document.querySelector('.knowledge-desk-toolbar'),
      treeVisible: !!document.querySelector('.knowledge-desk-tree'),
    }))
    checks.push({
      id: 'narrow-layout-no-horizontal-overflow',
      pass: narrow.bodyWidth <= narrow.viewport + 1 && narrow.toolbarVisible && narrow.treeVisible,
      detail: narrow,
    })
    await window.screenshot({ path: path.join(SHOTS, 'knowledge-home-layout-narrow.png'), scale: 'css' })
    checks.push({
  ...
  const report = {
    change: 'polish-knowledge-home-layout',
