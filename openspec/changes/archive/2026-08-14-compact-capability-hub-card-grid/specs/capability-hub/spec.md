...
Capability Hub MUST 为专家、技能和 MCP 连接器提供统一的 KnowMe 浅色暖灰视觉系统。页面 MUST 包含与工作台一致的单层外部顶部菜单栏、顶部搜索、同页类型 Tab、分类筛选、已安装筛选、可选精选区、响应式能力目录和右侧详情抽屉；在常见桌面窗口宽度下，精选区与能力目录 MUST 使用三列卡片宽度以保持紧凑、可扫描的信息层级，窗口变窄时 MUST 逐级降为两列和单列且不产生横向溢出。工作区宿主顶部栏 MUST 按“能力图标与能力 Hub 标题 → 类型 Tab → 右侧操作”的顺序组织，类型 Tab MUST 使用与工作台一级页签一致的分组底板、纯文字标签和绿色实底选中态。Hub 内嵌页面 MUST 隐藏自身重复菜单栏，内容区 MUST NOT 再重复展示英文眉题、当前类型大标题、总数徽标或介绍文案。可交互元素 MUST 提供悬停、按压与键盘焦点反馈，目录加载、无结果及错误 MUST 显示与页面结构匹配的状态。
...
#### Scenario: Common desktop width uses compact cards

- **WHEN** 用户在常见桌面窗口宽度下浏览专家、技能或 MCP 连接器
- **THEN** 精选区和能力目录均以三列卡片宽度展示
- **AND** 精选条目不足三项时现有卡片不拉伸占满缺失列

#### Scenario: Hub adapts to window width

- **WHEN** Hub 宽度从宽桌面缩小到中等和窄桌面窗口
- **THEN** 精选区与能力目录依次从三列降为两列和单列，标题、搜索、类型 Tab 与筛选按可用空间重排
- **AND** 页面不产生横向滚动，卡片文本和核心操作保持可见
